...
function subscribersPath(root) {
  return root.endsWith(".json") ? root : `${root}/${FILE}`
}
...
export async function loadSubscribers(root) {
  try {
    const parsed = JSON.parse(await Deno.readTextFile(subscribersPath(root)))
...
export async function saveSubscribers(root, subscribers) {
  const target = subscribersPath(root)
  const tmp = `${target}.tmp`
  await Deno.writeTextFile(tmp, JSON.stringify(subscribers, null, 2))
  await Deno.rename(tmp, target)
}

// One @, non-empty local part, a dotted domain with a 2+ char TLD, no spaces.
...
// Stamp the last time a confirmation email went out to this address, so the
// caller can enforce a per-address cooldown and never let the form be used to
// bomb one inbox. Persisted, so the cap survives restarts.
export async function markConfirmationSent(root, email) {
  const normalized = email.trim().toLowerCase()
  const subscribers = await loadSubscribers(root)
  const entry = subscribers.find((s) => s.email === normalized)
  if (!entry) return null
  entry.confirmation_sent_at = new Date().toISOString()
  await saveSubscribers(root, subscribers)
  return entry
}
...
export function unsubscribeUrl(siteOrToken, maybeToken) {
  const token = maybeToken || siteOrToken
  const baseUrl = maybeToken ? siteOrToken.baseUrl : "https://evbogue.com"
  return `${baseUrl}/unsubscribe?token=${token}`
}

export function confirmUrl(siteOrToken, maybeToken) {
  const token = maybeToken || siteOrToken
  const baseUrl = maybeToken ? siteOrToken.baseUrl : "https://evbogue.com"
