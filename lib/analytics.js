...
const DEFAULT_NAMESPACE = "evbogue"
...
function analyticsPath(root, namespace = DEFAULT_NAMESPACE) {
  return `${root}/${ANALYTICS_DIR}/${namespace}.jsonl`
}

export async function recordView(root, { slug, kind = "view", userAgent = "", ip = "", salt = "" }, namespace = DEFAULT_NAMESPACE) {
  ...
  const path = analyticsPath(root, namespace)
  try {
...
export async function loadViews(root, namespace = DEFAULT_NAMESPACE) {
  const path = analyticsPath(root, namespace)
  try {
    ...
    if (namespace === DEFAULT_NAMESPACE) {
      try {
        const text = await Deno.readTextFile(`${root}/${ANALYTICS_DIR}/views.jsonl`)
        return text.split("\n").filter(Boolean).map((line) => {
          try { return JSON.parse(line) } catch { return null }
        }).filter(Boolean)
      } catch {
...
export async function recordEvent(root, { kind, ...fields }, namespace = DEFAULT_NAMESPACE) {
