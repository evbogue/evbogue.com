...
export async function sendAdminNotification(event, entry) {
  const t = transporter();
  if (!t) {
  ...
  await t.sendMail({
    from: `evbogue.com <${SMTP_USER}>`,
    to: SMTP_USER,
    subject: `[evbogue.com] ${verb}: ${entry.email}`,
    text: lines.join("\n"),
  });
}

export async function sendWeeklyReport(weekLabel, markdownContent) {
  const t = transporter();
  if (!t) {
  ...
  await t.sendMail({
    from: `evbogue.com <${SMTP_USER}>`,
    to: SMTP_USER,
    subject: `[evbogue.com] weekly report: ${weekLabel}`,
    text: markdownContent,
  });
}

export async function sendReconfirmation(entry) {
  const t = transporter();
  if (!t) {
  ...
  await t.sendMail({
    from: `Ev Bogue <${SMTP_USER}>`,
    to: entry.email,
    subject: "Stay subscribed to evbogue.com",
    text: `Hey —

I just rolled out a confirmation step for evbogue.com subscribers. To keep getting dispatches, click the link below:
...
Unsubscribe: ${unsub}`,
    html: `<p>Hey —</p>
<p>I just rolled out a confirmation step for evbogue.com subscribers. To keep getting dispatches, click the link below:</p>
<p><a href="${confirm}">${confirm}</a></p>
